...
def test_parse_csv_full_row(tmp_path: Path) -> None:
    ...
    csv_path.write_text(
        ...
        "The Housemaid,2026-01-03,14:00,16:32,cinema,Tuschinski,https://www.imdb.com/title/tt27543632/\n"
    )
...
*** Ready or Not 2: Here I Come - Movies
:PROPERTIES:
...
:END:
** Netflix :netflix:
*** Louis Theroux: Inside the Manosphere - Movies
<2026-03-14 Sat>
:PROPERTIES:
...
def test_parse_org_full_range_entry(tmp_path: Path) -> None:
    ...
    housemaid = next(r for r in rows if r.entry and r.entry.title == "The Housemaid")
...
def test_parse_org_start_only_entry(tmp_path: Path) -> None:
    ...
    mk2 = next(r for r in rows if r.entry and r.entry.title == "Mortal Kombat II")
    assert mk2.entry.start_time == time(16, 10)
    assert mk2.entry.end_time is None
    assert mk2.entry.venue == "Gsc Gurney Plaza Penang"


def test_parse_org_date_only_entry_from_netflix_medium(tmp_path: Path) -> None:
    rows = parse_org(_org_file(tmp_path))

    louis = next(r for r in rows if r.entry and "Louis Theroux" in r.entry.title)
...
def test_parse_org_recovers_venue_from_duplicate_properties_drawer(tmp_path: Path) -> None:
    ...
    rrn2 = next(r for r in rows if r.entry and "Ready or Not 2" in r.entry.title)
    assert rrn2.entry.venue == "Pathe Noord"
...
def test_run_import_creates_entries_and_resolves_medium_and_venue(store: Store) -> None:
    ...
    assert [v.name for v in store.list_venues()] == ["Tuschinski"]
...
def test_run_import_reuses_existing_medium_and_venue(store: Store) -> None:
    ...
    existing_venue = store.add_venue("Tuschinski")
    rows = [
        ParsedRow(
            row_number=1,
            entry=ImportRow(
                title="Dune", date=date(2026, 1, 1), medium="cinema", venue="Tuschinski"
            ),
            error=None,
        )
    ]
...
def test_run_import_skips_a_duplicate_by_default(store: Store) -> None:
    ...
    store.create_entry(
        title="Ready or Not 2: Here I Come", date=date(2026, 3, 29), medium_id=medium.id
    )
    rows = [
        ParsedRow(
            row_number=1,
            entry=ImportRow(
                title="Ready or Not 2: Here I Come - Movies",
                date=date(2026, 3, 29),
                medium="cinema",
            ),
            error=None,
